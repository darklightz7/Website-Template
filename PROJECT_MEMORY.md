...
- Performed complete header audit and responsive navigation overhaul across `css/style.css` and `js/app.js`:
  ...
  - **Dynamic Viewport Resizing & Mobile Header Fix**: Implemented CSS `clamp()` scaling across `.brand-logo` (font-size & SVG icon), `.nav-link` items, and `.nav-cta` action buttons. At `<= 768px`, phone CTA is hidden from top bar (accessible inside drawer). At `<= 440px`, `.brand-logo .text-accent` is hidden (`[Shield] BLACKHAWK`) with compact CTAs and 36px toggle, reducing header width to ~230px and guaranteeing the hamburger menu is never pushed off-screen across any mobile viewport (320px–440px+).
